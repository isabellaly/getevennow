 //
  ...
  var questionTemplateString = '\<div class="question-row" id="question<%= question_index %>"> \<%= question %><div><br>\<%  \_.each(answers, function(answer, answer_index) { \%> \<input type="radio" name="question<%= question_index %>" value="<%= answer_index %>">\<label for="<%= answer_index %>"><%= answer %></label><br>\<% })%>\</div>';

  var resultTemplateString = '\<a href="<%= insightLink %>">Insight</a><br>\<%= insightText %><br>\<p> <%= response %><\p>';
